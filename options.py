...
def get_options(option_type):
  ...
  """
  # Common
  tf.app.flags.DEFINE_string("env_type", "vrep", "environment type (lab or gym or maze)")
  tf.app.flags.DEFINE_string("env_name", "vrep_env",  "environment name")
  ...
  if option_type == 'training':
    ...
    tf.app.flags.DEFINE_string("log_file", "/tmp/unreal_log", "log file directory")
    tf.app.flags.DEFINE_float("initial_alpha_low", 1e-5, "log_uniform low limit for learning rate")
    tf.app.flags.DEFINE_float("initial_alpha_high", 3e-3, "log_uniform high limit for learning rate")
    ...
    tf.app.flags.DEFINE_integer("experience_history_size", 100, "experience replay buffer size")
    ...
    tf.app.flags.DEFINE_integer("save_interval_step", 2000, "saving interval steps")

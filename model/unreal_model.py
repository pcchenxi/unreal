...
class UnrealModel(object):
    ...
    def _base_feature_layers(self, state_input, reuse=None):
        with tf.variable_scope("base_feature", reuse=reuse) as scope:
            self.laser = tf.slice(state_input, [0, 0], [-1, 180])
            self.path = tf.slice(state_input, [0, 180], [-1, 2])
            ...
            conv1 = tf.layers.conv1d(   inputs=laser_reshape,
                                        ...
                                        name = 'base_laser_conv1')
            conv2 = tf.layers.conv1d(   inputs=conv1,
                                        filters=32,
                                        kernel_size=4,
                                        padding="same",
                                        activation=tf.nn.relu,
                                        name = 'base_laser_conv2')
            ...
            #                             padding="same",
            #                             activation=tf.nn.relu,
            #                             name = 'base_feature_conv3')
            self.fobs_size = 1 * 180 * 32

            conv_flat = tf.reshape(conv2, [-1, self.fobs_size])
            ...
            path_fc1 = tf.layers.dense(inputs=path_flat, units=32, activation=tf.nn.relu, name = 'base_path_fc1')
            path_fc2 = tf.layers.dense(inputs=path_fc1, units=32, activation=tf.nn.relu, name = 'base_path_fc2')

            feature = tf.concat([conv_flat, path_fc2], 1, name = 'base_laser_path_concat')
            feature_fc = tf.layers.dense(inputs=feature, units=32, activation=tf.nn.relu, name = 'base_final_fc')

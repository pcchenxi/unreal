...
from trainer.unreal_trainer import Trainer
from trainer.rmsprop_applier import RMSPropApplier
...
class Application(object):
  ...
  def train_function(self, parallel_index, preparing):
    ...
    while True:
      ...
      diff_global_t = trainer.process(self.sess,
                                      ...
                                      self.summary_writer, self.summary_op_score, self.merge_vr)
  ...
  def run(self):
    ...
    self.merge_vr = tf.summary.scalar("value_replay_loss", self.vr_loss_input)
  ...
  def save(self):
    ...
    path = flags.checkpoint_dir + '/' + 'checkpoint'
    print('Start saving.', path)
